...
| 资源 | 免费档 | 本系统用量估算 |
|---|---|---|
| Workers 请求 | ~10 万/天 | 30 人围观 × 4 小时 × 60s 轮询 ≈ 4.3 万请求/天，余量 2.3 倍（见下方 2026-09-24 更正） |
| D1 读 | 500 万行/天（**按账号计**，四个库共享） | 实测 143 万行/日（占账号池约 29%），见下节规约 |
| D1 存储 | 5GB | 全赛季数据 < 10MB |
| KV 读 | ~10 万/天 | 会话校验 + 缓存命中，够用 |

暴涨预案：围观人数远超预期时，第一步是把公开页缓存从 30s 延长并前置到 CDN 层（改一行配置）；第二步才是 $5/月 Workers Paid 档。

> 2026-09-24 更正：原写「每次轮询 1 个聚合接口（KV 缓存命中则不计），远低于上限」低估了实际形状——
> 实测本仓 24h 读 1,428,496 行 / 8,205 次读查询（约 174 行/查询，而全库仅约 1,745 行，等于每行每天被读约 820 遍），
> 且 21 个公开面按「每个 TTL 窗口都有请求」相加上限约 1,449 万行/日，是账号整池的 2.9 倍。
> 没炸只因流量有间歇。量化方法与全部读数见 `scripts/d1-read-audit/README.md`。
>
...
2026-09-21 `whl-club` 触顶的报错原文是 `Your account has exceeded D1's free tier daily row read limit`（措辞是 account）。

**治理判据**（取代 club 的「单次 ≥10,000 行」——本仓 55 个读面没有一个达标，照搬会得出「无需治理」的错误结论）：

> **单价 × 重算频率**。单价 ≥1,000 行且 TTL ≤60s（每日窗口数 ≥1,440）⇒ 上限 ≥144 万行/日，必做；
> 单价 ≥1,000 行但走 KV/长 TTL ⇒ 看实测频率再定；单价 <500 行 ⇒ 除非高频否则不动。

**查询侧硬要求**：

1. 排序键要与筛选列同源。`ORDER BY <未索引列>` 会让规划器放弃窄过滤改走全扫/临时 B 树排序
...
2. `OR` 不要作用在 JOIN 出来的列上（如 `he.team_id = ? OR ae.team_id = ?`）。规划器无法用索引，
   ...
   **但先看它是否在驱动扫描**：若外层已有 `WHERE m.id IN (...)`（rowid 列表），`OR` 只是在已取回的行上求值，
   ...
   「该侧未定队」排除会失效——`NULL IN (...)` 不为真，但 `OR` 的另一支可能为真，必须补
   `m.home_entry_id IS NOT NULL AND m.away_entry_id IS NOT NULL`。
3. 窄过滤 + 大 `LIMIT` 的组合要拆两段式：先只取 id（`LIMIT` 生效），再按主键补整行。
4. `LIMIT n` 后面还要 join 多张表时，先确认 `LIMIT` 是不是在 join 之后才生效。
5. 加 `INDEXED BY` 前先看执行计划——本仓多条查询是被规划器「优化」到更宽的索引上的
   （如 `fetchRoundFinished` 放弃 `idx_match_stage` 去用 `idx_match_status` 以省排序），
   强制窄索引反而省读。`INDEXED BY` 是硬指令，索引被删则查询直接报错。
6. **反直觉区**：相关子查询改「分组物化后 join」不一定更省——收益取决于驱动集合占全表的比例
   （赛事页 12 支参赛队驱动时 +68%，管理端 20 队时 +12%，一律更贵）。这类改写必须**先量再改**。
7. 合并端点时必须**按 TTL 分组**，不能把短 TTL 的面并进长 TTL 的聚合端点。反例：把
   `/api/public/live`（60s）并进 `/api/public/home` 会让整个聚合按 60s 重算，
   feed 单段 4,315 行 × 1,440 窗 = **621 万行/日 > 账号整池 500 万**。
   正确做法是把同 TTL 的面合成一个端点（`/api/public/home` = 300s 组，`/api/public/live` 独立），
   前端一次 `Promise.all` 发两个请求。合并的收益在**请求数**（免费档 10 万请求/日比行读更早撞墙），
   不在行读——行读只由「单价 × 重算频率」决定，端点个数不影响。
...
**禁止用以下三条通道判断 D1 是否可用**（都会在配额触顶时仍然返回成功，给出假安全感）：

- `/api/health`（只读 `sqlite_schema`，不碰业务表）；
- `wrangler d1 execute --remote` 管理通道（配额触顶期间照样能跑）；
- 任何固定缓存键且吞掉错误的端点。

要看真实读数用 `npx wrangler d1 info <db> --json`（不消耗行读）或 D1 Analytics。

**豁免清单与重估触发条件**：已判定「不动」的形状见 `scripts/d1-read-audit/README.md` §9 C 档。
其中「加索引」是条件性豁免——当前「索引够用」建立在 `match` 仅 218 行上；
当 `match` 涨到 2,000 行量级时重跑形状普查，或写配额逼近 10 万行/日时重算索引写成本。

**迁移纪律**：迁移一旦 apply 到生产就不得再改（club 经验）；表达式索引必须与排序表达式**逐字一致**，
否则优化器静默不用。本仓写配额实测仅 2,335 行/日（上限 10 万），索引写放大不构成约束。
