...
> **读消耗的量化与治理看别处**（2026-09-24 起）：本文件的口径是**延迟**（每串行 D1 查询 ≈ +0.2s），
> 不测行读量。D1 **行读消耗**的实测基线、形状排行、A/B 改写对照与治理判据见
> `scripts/d1-read-audit/README.md`；成文规约见 `TECH_DESIGN.md` §3.1「D1 读消耗规约」。
> 两者互补：这里省往返，那边省扫描行数。
...
> **部署待生效项：Smart Placement**（wrangler.jsonc `"placement": {"mode": "smart"}`，2026-09-06 配置）——CF 自动把 Worker 计算挪近亚太 D1 库，削减跨洋查询往返；部署后 ~15 分钟热身，dash 里 Worker 的 Smart Placement 状态可查。
