...
日期：2026-09-18（2026-09-19 走查后追加第 13 步调整）
状态：已实现并走查通过
...
- ② **战术设计**：战术设置（阵型/组织风格/防线高度）+ **队长与定位球（原「球员指派」，本卡收尾块）** + 战术码（现 `tac-code-panel`）
...
**移动端（≤979px）**：页签吸顶可横滚、模式徽标排在页签最前（`order:-1`，否则会被推出视口）；球场常驻最上、右栏卡接在其后；现有 6 项 order 链（archives/code/import）因卡片换父容器而失效，改由 `.tac-side > *` 统一管顺序。
...
- 5 组在卡内的**渲染顺序**：队长 → 界外球 → 任意球 → 角球进攻 → 角球防守（队长组只有 1 项，界外球的 2 项正好并到它旁边填空白）。组顺序表 `ASSIGN_RENDER_ORDER` 按组名取，组名改了自动退回 `ASSIGN_GROUPS` 原顺序；存储与 DTO 始终按 `ASSIGN_GROUPS` 原序。
...
- 「队长与定位球」块（在 `card tac-settings` 内、卡尾，上分隔线隔开）：块头（`h3` + `small` 出处 + 展开/收起 + 冲突或已填计数）+ 摘要 + 5 组自适应网格（`auto-fit minmax(230px,1fr)`，宽屏 2 列让队长与界外球并排、≤560px 单列）+ 冲突提示与「清除冲突项」。
