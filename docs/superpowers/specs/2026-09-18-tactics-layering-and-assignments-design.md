# 战术板：UI 分层 + 球员指派（FC26 Assignments）
...
2. **新增球员指派（Assignments）**：照 FC26 的「球队管理 → 指派」口径做 5 组 18 项，随阵容提交落库，教练端可回显、管理端赛前备案可见、开赛后公开端阵容里显示。
...
| 指派归属哪一层 | ① 本场备案（判据：它随「本场的人」变，战术码带不走它，候选池＝本场首发） |
| 指派互斥强度 | 互斥 + **拦提交**（卡头红标 + 标出冲突项 + 一键清除） |
...
**常驻核心**（两种模式共用，不进层）
- 左列 400px 球场卡（`.tac-pitch-panel`）：磁贴显示号码/姓名、停赛伤停标记、**队长 C 角标**、**位置重复黄标**；点磁贴选中位置。
- 右栏顶部**选中位置编辑器**（现 `tac-editor`）：这个位置放谁 + 他在打法里的角色与重心。理由：备案问「这儿放谁」，设计问「这个人干什么」，同一个动作，一步到位。

**三个层面板**（原卡片整体搬入，内部实现不动，只换父容器）
- ① **本场备案**：选择目标比赛 + 伤停与停赛（现 `tac-submit` 左列/右列，去掉提交按钮）→ 首发与替补（现页尾 `tac-bench` 九格）→ **球员指派（新）** → 提交（拆出的独立窄卡）
- ② **战术设计**：战术设置（阵型/组织风格/防线高度）+ 战术码（现 `tac-code-panel`）
- ③ **工具与档案**：导入战术码（`tac-import-panel`）+ 存档（`tac-archives`）

**层状态记 URL query**：`/tactics?zone=lineup|design|tools`。刷新/分享/前进后退都对，不污染 `ftc26-*` 草稿键。视觉沿用仓库现有 `.tabs`/`.tab-active`（`src/styles.css:258-277`）。

**移动端（≤979px）**：层条吸顶可横滚；球场常驻最上、编辑器次之、面板接在其后；现有 6 项 order 链（archives/code/import）因卡片换父容器而失效，简化为「球场 0／编辑器 1／面板 2」。
...
- 指派卡默认折叠、记住展开状态；卡头常显摘要（已填 n/18 · 队长 #7 张三 · 角球 ←李四 →李四）+ 冲突红标。
- 指派指向的球员已不在首发时**保留并标黄「已不在首发」**，不静默清空。
- **没填完不拦提交**（与「伤停只提示不拦截」一致）；**互斥冲突拦提交**。
- 已提交场次给只读摘要 + 「载入本次已提交的指派」按钮，**不自动预填**（与现有 `names` 行为一致，避免半套数据误提交）。
...
- 指派卡：折叠头 + 摘要 + 5 组自适应网格（宽屏 `auto-fit` 多列、≤560px 单列）+ 冲突提示与「清除冲突项」+ 已提交摘要与「载入本次已提交的指派」。
- `lineupProblem()` 末尾加互斥预检（拦提交）。
- 球场磁贴：队长 C 角标；位置重复（首发一人占两个位置，含与替补的判重口径）置黄；点中球员时编辑器列出他担任的指派。

### 4.6 三端展示

- `src/components/LineupView.tsx`：`LineupSide` 首发行在队长名后插 C 标，`lu-meta` 前渲染新的 `AssignList`（按组列已填项、队长最前、非本场标「非本场」）。调用点只有管理端 `src/pages/MatchesTab.tsx` 与公开端 `src/pages/PublicMatchDetail.tsx` ⇒ **改一处三端生效**。
...
方案：代打模式草稿落到 `ftc26-proxy-<mid>-state-v1` / `-names-v1`（每场一份，切场次各留各的），切回本队恢复 `ftc26-*`；`resetAll()` 与指派草稿一并对两种模式清空。
...
- 回显：`GET /proxy/:mid/board` 的 `lineup` 是 `TeamLineupDTO` ⇒ 一加 `assign`，代打者打开就看到目标队已提交的指派（含代打留痕），可「载入本次已提交的指派」回填。
...
7. 前端指派卡（状态/LS/存档回填/resetAll/互斥预检/submitLineup body）。
8. 前端分层 + 代打保真（层条含模式徽标、三面板、拆提交卡、①层门槛、身份切换器原位、③层代打提示、URL query、移动端 order、新样式）。
9. 草稿按模式隔离。
