...
// 伤情条／周报的缺阵尾巴。两池事实口径写死，只换说法：
// 有尾巴池 = 本轮（本周）受伤的人里还有 n 人挂着没打完的缺阵场次；
// 清空池 = 一个都没有（这几个人不再缺阵后续比赛，但登记表没写复出，所以不下「伤愈／全员健康」的结论）。
export const INJURY_OUT_TAIL: ((n: number) => string)[] = [
  (n) => `，另有 ${n} 人仍在伤停`,
  (n) => `，另有 ${n} 人继续缺阵`,
  (n) => `，其中 ${n} 人尚未复出`,
  (n) => `，还有 ${n} 人留在伤停名单上`,
...
export const INJURY_CLEAR_TAIL: string[] = [
  "，无人仍在伤停",
  "，伤停名单上已无人在列",
  "，这批伤员都不再缺阵",
  "，没有伤员继续缺阵",
